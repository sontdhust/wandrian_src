...
#include "../include/extended_core.hpp"
...
bool ExtendedCore::initialize() {
  ...
  laser_subscriber = nh.subscribe<sensor_msgs::LaserScan>("laser", 1,
      &ExtendedCore::subscribe_laser, this);
  ...
  thread_keyboard.start(&ExtendedCore::start_thread_keyboard, *this);
  return true;
}

void ExtendedCore::spin() {
...
void ExtendedCore::stop() {
  velocity->linear.x = 0.0;
  velocity->angular.z = 0.0;
  velocity_publisher.publish(velocity);
}

std::string ExtendedCore::get_plan_name() {
  return plan_name;
}

double ExtendedCore::get_starting_point_x() {
  return starting_point_x;
}

double ExtendedCore::get_starting_point_y() {
  return starting_point_y;
}

double ExtendedCore::get_robot_size() {
  return robot_size;
}

PointPtr ExtendedCore::get_current_position() {
  return current_position;
}

VectorPtr ExtendedCore::get_current_orientation() {
  return current_orientation;
}

bool* ExtendedCore::get_obstacles() {
  return obstacles;
}

double ExtendedCore::get_linear_velocity_step() {
  return linear_velocity_step;
}

double ExtendedCore::get_angular_velocity_step() {
  return angular_velocity_step;
}

void ExtendedCore::set_behavior_run(boost::function<void()> behavior_run) {
  this->behavior_run = behavior_run;
}

void ExtendedCore::set_linear_velocity(double linear_velocity) {
  velocity->linear.x = linear_velocity;
}

void ExtendedCore::set_angular_velocity(double angular_velocity) {
...
void ExtendedCore::run() {
  if (behavior_run)
    behavior_run();
}

void ExtendedCore::start_thread_keyboard() {
  struct termios raw;
...
void ExtendedCore::process_keyboard_input(char c) {
  switch (c) {
...
void ExtendedCore::start_thread_run() {
  run();
}

void ExtendedCore::enable_power() {
  stop();
...
void ExtendedCore::disable_power() {
  stop();
...
void ExtendedCore::subscribe_odometry(const nav_msgs::OdometryConstPtr& odom) {
...
void ExtendedCore::subscribe_laser(const sensor_msgs::LaserScanConstPtr& laser) {
