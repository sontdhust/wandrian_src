...
Polygon::Polygon(std::list<PointPtr> points) : points(points), graph() {
  std::list<PointPtr>::iterator point = this->points.begin();
  while (point != this->points.end()) {
    std::list<PointPtr>::iterator next =
        boost::next(point) != this->points.end() ? boost::next(point) : this->points.begin();
    if (*next == *point) {
...
Polygon::~Polygon() {
  points.clear();
  for (std::map<PointPtr, std::set<PointPtr, PointComp> >::iterator n = graph.begin(); n != graph.end(); n++) {
    for (std::set<PointPtr>::iterator p = n->second.begin(); p != (*n).second.end(); p++) {
      n->second.erase(p);
    }
    graph.erase(n);
  }
...
std::list<PointPtr> Polygon::get_points() { return points; }
...
std::list<PointPtr> Polygon::get_boundary() {
  ...
  for (std::list<PointPtr>::iterator u = upper_boundary.begin(); u != upper_boundary.end(); u++) {
    boundary.insert(boundary.end(), PointPtr(new Point(**u)));
  }

  for (std::list<PointPtr>::reverse_iterator l = boost::next(lower_boundary.rbegin());
       boost::next(l) != lower_boundary.rend(); l++) {
...
void Polygon::build() {
  for (std::list<PointPtr>::iterator current = points.begin(); current != points.end(); current++) {
    // Insert current point into graph if not yet
    if (graph.find(*current) == graph.end())
      graph.insert(std::pair<PointPtr, std::set<PointPtr, PointComp> >(*current, std::set<PointPtr, PointComp>()));
    // Find next point
    std::list<PointPtr>::iterator next;
    ...
    if (graph.find(*next) == graph.end())
      graph.insert(std::pair<PointPtr, std::set<PointPtr, PointComp> >(*next, std::set<PointPtr, PointComp>()));
    // Create edge
    if (current != next) {
  ...
  for (std::map<PointPtr, std::set<PointPtr, PointComp> >::iterator current = graph.begin(); current != graph.end();
       current++) {
    for (std::set<PointPtr>::iterator current_adjacent = current->second.begin();
         current_adjacent != current->second.end(); current_adjacent++) {
      for (std::map<PointPtr, std::set<PointPtr, PointComp> >::iterator another = boost::next(current);
           another != graph.end(); another++) {
        for (std::set<PointPtr>::iterator another_adjacent = another->second.begin();
             another_adjacent != another->second.end(); another_adjacent++) {
          SegmentPtr current_segment = SegmentPtr(new Segment(current->first, *current_adjacent));
          SegmentPtr another_segment = SegmentPtr(new Segment(another->first, *another_adjacent));
          PointPtr intersect = current_segment % another_segment;
          if (intersect) {
            if (segments.find(current_segment) == segments.end())
              segments.insert(std::pair<SegmentPtr, std::set<PointPtr, PointComp> >(current_segment,
                                                                                   std::set<PointPtr, PointComp>()));
            if (segments.find(another_segment) == segments.end())
              segments.insert(std::pair<SegmentPtr, std::set<PointPtr, PointComp> >(another_segment,
                                                                                   std::set<PointPtr, PointComp>()));
  ...
  for (std::map<SegmentPtr, std::set<PointPtr, PointComp> >::iterator segment = segments.begin();
       segment != segments.end(); segment++) {
    for (std::set<PointPtr>::iterator intersect = segment->second.begin(); intersect != segment->second.end();
         intersect++) {
      ...
      if (graph.find(*intersect) == graph.end())
        graph.insert(std::pair<PointPtr, std::set<PointPtr, PointComp> >(*intersect, std::set<PointPtr, PointComp>()));
      graph.find(*intersect)->second.insert(segment->first->p1);
      graph.find(*intersect)->second.insert(segment->first->p2);
      for (std::set<PointPtr>::iterator another = segment->second.begin(); another != segment->second.end();
           another++) {
...
PointPtr Polygon::get_leftmost() {
  ...
  for (std::list<PointPtr>::iterator current = boost::next(points.begin()); current != points.end(); current++) {
    if (*current < leftmost)
      leftmost = *current;
  }
  return leftmost;
...
std::list<PointPtr> Polygon::get_upper_boundary() { return get_partial_boundary(true); }

std::list<PointPtr> Polygon::get_lower_boundary() { return get_partial_boundary(false); }
...
std::list<PointPtr> Polygon::get_partial_boundary(bool is_upper) {
  ...
  while (current != rightmost) {
    ...
    if (is_upper)
      ...
      angle = 0;
    PointPtr next;
    for (std::set<PointPtr>::iterator adjacent = graph.find(current)->second.begin();
         adjacent != graph.find(current)->second.end(); adjacent++) {
      double a = std::atan2(previous->y - current->y, previous->x - current->x) -
                 std::atan2((*adjacent)->y - current->y, (*adjacent)->x - current->x);
      double d = std::sqrt(std::pow(current->x - (*adjacent)->x, 2) + std::pow(current->y - (*adjacent)->y, 2));
      if (is_upper) {
        a = std::abs(a) <= EPSILON ? 2 * M_PI : a > 0 ? a : 2 * M_PI + a;
        if (a - angle < -EPSILON || (std::abs(a - angle) < EPSILON && d < distance)) {
          angle = a;
          distance = d;
          next = *adjacent;
        }
