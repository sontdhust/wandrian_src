...
SpiralStc::SpiralStc() : tool_size(0) {}
...
void SpiralStc::initialize(PointPtr starting_point, double tool_size) {
  ...
  starting_cell = CellPtr(new Cell(
      PointPtr(new Point(starting_point->x - tool_size / 2, starting_point->y + tool_size / 2)), 2 * tool_size));
  starting_cell->set_parent(CellPtr(
      new Cell(PointPtr(new Point(starting_cell->get_center()->x, starting_cell->get_center()->y - 2 * tool_size)),
               2 * tool_size)));
...
void SpiralStc::set_behavior_see_obstacle(boost::function<bool(VectorPtr, double)> behavior_see_obstacle) {
...
void SpiralStc::scan(CellPtr current) {
  ...
  do {
    ...
    CellPtr neighbor = CellPtr(new Cell(current->get_center() + direction * 2 * tool_size, 2 * tool_size));
