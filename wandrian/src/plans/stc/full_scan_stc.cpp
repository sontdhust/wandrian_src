  ...
  if (~direction == AT_RIGHT_SIDE || ~direction == AT_LEFT_SIDE) {
    ...
    if (state_of(neighbor) != OLD) { // 'neighbor' is new cell
      ...
      PartiallyOccupiableCellPtr current_cell = PartiallyOccupiableCellPtr(
          new PartiallyOccupiableCell(neighbor->get_center() + -(-d) * 2 * tool_size, 2 * tool_size));
      ...
      bool cp =
          (position != current_position && see_obstacle(position - current_position, position % current_position));
      bool np = see_obstacle(neighbor_position - current_position, neighbor_position % current_position);
      bool vp = see_obstacle(vertical_position - current_position, vertical_position % current_position);
      bool dp = see_obstacle(diagonal_position - current_position, diagonal_position % current_position);
      if (cp)
        std::cout << "cp ";
      if (np)
        std::cout << "np ";
