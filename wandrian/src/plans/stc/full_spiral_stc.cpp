...
bool FullSpiralStc::state_of_subcells_of(CellPtr cell, Orientation orientation) {
  ...
  PartiallyOccupiableCellPtr c = boost::static_pointer_cast<PartiallyOccupiableCell>(*i);
  Quadrant q;
...
bool FullSpiralStc::should_go_to(CellPtr neighbor, VectorPtr direction) {
  return (state_of(neighbor) != OLD) || (state_of_subcells_of(neighbor, ~direction) == DIAGONALLY_OPPOSITE);
}
