...
BoustrophedonOnline::BoustrophedonOnline()
    : tool_size(0), check_rotate(0), straight(false), number_cell(0), number_neighbor_cell(0), check_insert(0),
      start(vertex(0)), goal(vertex(0)) {}

BoustrophedonOnline::~BoustrophedonOnline() {}

void BoustrophedonOnline::initialize(PointPtr starting_point, double tool_size) {
  ...
  starting_cell = CellPtr(new Cell(PointPtr(new Point(starting_point->x, starting_point->y)), tool_size));
  starting_cell->set_parent(CellPtr(new Cell(
      PointPtr(new Point(starting_cell->get_center()->x, starting_cell->get_center()->y - tool_size)), tool_size)));
...
void BoustrophedonOnline::set_behavior_see_obstacle(boost::function<bool(VectorPtr, double)> behavior_see_obstacle) {
...
bool BoustrophedonOnline::go_with(VectorPtr direction, double distance) {
  ...
  PointPtr new_position = PointPtr(new Point(last_position + direction * distance));
  return go_to(new_position);
}

bool BoustrophedonOnline::go_with_bpcell(PointPtr last_position, VectorPtr direction, double distance) {
  PointPtr new_position = PointPtr(new Point(last_position + direction * distance));
  return go_to(new_position);
}

void BoustrophedonOnline::go_straight(CellPtr neighbor, CellPtr current, VectorPtr direction) {
  std::cout << "\n";
...
void BoustrophedonOnline::turn_left(CellPtr neighbor_left, CellPtr current, VectorPtr direction) {
  std::cout << "\n";
...
void BoustrophedonOnline::turn_right(CellPtr neighbor_right, CellPtr current, VectorPtr direction) {
  std::cout << "\n";
...
void BoustrophedonOnline::bpmove(CellPtr current) {
  ...
  for (++spi; spi != backtrack_path.end(); ++spi) {
    CellPtr next_cell = CellPtr(new Cell(PointPtr(new Point(locations[*spi].x, locations[*spi].y)), tool_size));
    VectorPtr direction = VectorPtr(new Vector((next_cell->get_center() - current->get_center()) / tool_size));
    go_with_bpcell(current->get_center(), direction, tool_size);
    current = next_cell;
  }
  VectorPtr direction = VectorPtr(new Vector((starting_cell->get_center() - current->get_center()) / tool_size));
  go_with_bpcell(current->get_center(), direction, tool_size);
...
void BoustrophedonOnline::scan(CellPtr current) {
  ...
  VectorPtr direction =
      VectorPtr(new Vector((current->get_parent()->get_center() - current->get_center()) / tool_size));

  // Quay vector 180 do
  direction = +(+direction);

  CellPtr neighbor = CellPtr(new Cell(PointPtr(new Point(current->get_center() + direction * tool_size)), tool_size));
  ...
  neighbor = CellPtr(new Cell(PointPtr(new Point(current->get_center() + direction * tool_size)), tool_size));
  if (state_of(neighbor) != OLD && see_obstacle(direction, tool_size / 2) == false) {
    bplist.insert(neighbor);
  }

  CellPtr neighbor_left =
      CellPtr(new Cell(PointPtr(new Point(current->get_center() + +direction * tool_size)), tool_size));
  if (state_of(neighbor_left) != OLD && see_obstacle(+direction, tool_size / 2) == false) {
    bplist.insert(neighbor_left);
  }

  CellPtr neighbor_right =
      CellPtr(new Cell(PointPtr(new Point(current->get_center() + -direction * tool_size)), tool_size));
  if (state_of(neighbor_right) != OLD && see_obstacle(-direction, tool_size / 2) == false) {
    bplist.insert(neighbor_right);
  }
  CellPtr neighbor_bottom =
      CellPtr(new Cell(PointPtr(new Point(*(current->get_center() + +(+direction) * tool_size))), tool_size));
  if (state_of(neighbor_bottom) != OLD && see_obstacle(+(+direction), tool_size / 2) == false) {
  ...
  if (straight == false) {
    if (check_rotate == 1) {
      if (see_obstacle(-direction, tool_size / 2) == false && state_of(neighbor_right) != OLD) {
        straight = true;
        turn_right(neighbor_right, current, direction);
      } else if (see_obstacle(+direction, tool_size / 2) == false && state_of(neighbor_left) != OLD) {
        straight = true;
        turn_left(neighbor_left, current, direction);
      } else if (see_obstacle(direction, tool_size / 2) == false && state_of(neighbor) != OLD) {
        go_straight(neighbor, current, direction);
      }
    } else {
      if (see_obstacle(+direction, tool_size / 2) == false && state_of(neighbor_left) != OLD) {
        straight = true;
        turn_left(neighbor_left, current, direction);
      } else if (see_obstacle(-direction, tool_size / 2) == false && state_of(neighbor_right) != OLD) {
        straight = true;
        turn_right(neighbor_right, current, direction);
      } else if (see_obstacle(direction, tool_size / 2) == false && state_of(neighbor) != OLD) {
        go_straight(neighbor, current, direction);
    ...
    } else if (see_obstacle(direction, tool_size / 2) == false && state_of(neighbor) != OLD) { // New free neighbor
      straight = true;
      go_straight(neighbor, current, direction);
    }
  }
  old_cells.insert(old_cells.end(), current);
  std::cout << "\n\033[1;34mcurrent-\033[0m\033[1;31mEND\033[0m: " << current->get_center()->x << ","
            << current->get_center()->y << "\n";
  if (bplist.size() > 0) {
    refine_bplist();
    std::cout << "Backtrack list: " << bplist.size() << "\n";
...
void BoustrophedonOnline::find_bpcell(CellPtr current) {
  std::cout << current->get_center()->x << ", " << current->get_center()->y << std::endl;
  start = check_vertex(current);
  int time = 100;
  for (std::set<CellPtr>::iterator i = bplist.begin(); i != bplist.end(); i++) {
    CellPtr tmp = CellPtr(*i);
    CellPtr neighbor_E =
        CellPtr(new Cell(PointPtr(new Point(tmp->get_center()->x - tool_size, tmp->get_center()->y)), tool_size));
    ...
    CellPtr neighbor_W =
        CellPtr(new Cell(PointPtr(new Point(tmp->get_center()->x + tool_size, tmp->get_center()->y)), tool_size));
    if (state_of(neighbor_W) == OLD && check_distance(current, neighbor_W) < check_distance(current, neighbor_E)) {
      goal = check_vertex(neighbor_W);
    }

    CellPtr neighbor_N =
        CellPtr(new Cell(PointPtr(new Point(tmp->get_center()->x, tmp->get_center()->y + tool_size)), tool_size));
    if (state_of(neighbor_N) == OLD) {
      if (state_of(neighbor_E) == OLD && check_distance(current, neighbor_N) < check_distance(current, neighbor_E)) {
        goal = check_vertex(neighbor_N);
      }
      if (state_of(neighbor_W) == OLD && check_distance(current, neighbor_N) < check_distance(current, neighbor_W)) {
        goal = check_vertex(neighbor_N);
      }
    }
    ...
    try {
      ...
      boost::astar_search(g, start, distance_heuristic<mygraph_t, cost, location *>(locations, goal),
  ...
  std::cout << "\033[1;32mNew Starting Cell:\033[0m"
            << " " << starting_cell->get_center()->x << ", " << starting_cell->get_center()->y << std::endl;
  std::list<vertex>::iterator spi = backtrack_path.begin();
  for (++spi; spi != backtrack_path.end(); ++spi)
    std::cout << "->" << locations[*spi].x << ", " << locations[*spi].y << std::endl;
}
...
void BoustrophedonOnline::insert_edge(CellPtr current, CellPtr neighbor, int insert) {
  edge_descriptor e;
  bool inserted;
  if (insert == 2) {
    boost::tie(e, inserted) =
        add_edge(edge(number_cell, check_vertex(neighbor)).first, edge(number_cell, check_vertex(neighbor)).second, g);
    std::cout << "Check insert: " << boost::edge(number_cell, check_vertex(neighbor), g).first << std::endl;
  } else {
    boost::tie(e, inserted) =
        add_edge(edge(number_cell, number_neighbor_cell).first, edge(number_cell, number_neighbor_cell).second, g);
    std::cout << "Check insert: " << boost::edge(number_cell, number_neighbor_cell, g).first << std::endl;
  }
  weightmap[e] = 1;
...
void BoustrophedonOnline::insert_cell_to_graph(CellPtr current, CellPtr neighbor, int vertex_current) {
