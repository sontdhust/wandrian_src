...
MstcOnline::MstcOnline() : tool_size(0) {}
...
void MstcOnline::set_behavior_see_obstacle(boost::function<bool(VectorPtr, double)> behavior_see_obstacle) {
...
State MstcOnline::state_of(CellPtr cell) {
  State state = (communicator->find_old_cell(boost::static_pointer_cast<IdentifiableCell>(cell))) ? OLD : NEW;
  if (state == OLD)
