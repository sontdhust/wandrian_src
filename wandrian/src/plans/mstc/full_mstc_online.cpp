...
void FullMstcOnline::initialize(PointPtr starting_point, double tool_size, CommunicatorPtr communicator) {
  this->tool_size = tool_size;
  this->communicator = communicator;
  // Initialize starting_cell
  starting_cell = PartiallyOccupiableIdentifiableCellPtr(new PartiallyOccupiableIdentifiableCell(
      PointPtr(new Point(starting_point->x - tool_size / 2, starting_point->y + tool_size / 2)), 2 * tool_size,
      communicator->get_robot_name()));
  starting_cell->set_parent(PartiallyOccupiableIdentifiableCellPtr(new PartiallyOccupiableIdentifiableCell(
...
State FullMstcOnline::state_of(CellPtr cell) {
  State state =
      (communicator->cells.find(boost::static_pointer_cast<IdentifiableCell>(cell)) != communicator->cells.end()) ? OLD
                                                                                                                  : NEW;
  if (state == OLD)
...
void FullMstcOnline::scan(CellPtr current) {
  ...
  VectorPtr orientation = (current->get_parent()->get_center() - current->get_center()) / 2 / tool_size;
  VectorPtr initial_orientation = orientation++;
  // Check current cell has diagonally opposite obstacles or not
  PartiallyOccupiableIdentifiableCellPtr c = boost::dynamic_pointer_cast<PartiallyOccupiableIdentifiableCell>(current);
  ...
  do {
    ...
    PartiallyOccupiableIdentifiableCellPtr neighbor =
        PartiallyOccupiableIdentifiableCellPtr(new PartiallyOccupiableIdentifiableCell(
            current->get_center() + orientation * 2 * tool_size, 2 * tool_size, communicator->get_robot_name()));
    std::cout << "  \033[1;33mneighbor:\033[0m " << neighbor->get_center()->x << "," << neighbor->get_center()->y;
    if (state_of(neighbor) == OLD &&
        (state_of_subcells_of(neighbor, ~orientation) == NON_DIAGONALLY_OPPOSITE_OBSTACLES)) { // Old cell
      // Go to next sub-cell
  ...
  } while (orientation++ % initial_orientation != (is_starting_cell ? IN_FRONT : AT_RIGHT_SIDE));
...
bool FullMstcOnline::go_from(CellPtr current, bool pass, CellPtr next) {
  VectorPtr orientation = (next->get_center() - current->get_center()) / (2 * tool_size);
  PartiallyOccupiableIdentifiableCellPtr c = boost::dynamic_pointer_cast<PartiallyOccupiableIdentifiableCell>(current);
  PartiallyOccupiableIdentifiableCellPtr n = boost::dynamic_pointer_cast<PartiallyOccupiableIdentifiableCell>(next);
  ...
  if (quadrant == q1 || quadrant == q2) {
    if (!see_obstacle(orientation, tool_size / 2)) {
      ...
      if (!see_obstacle(quadrant == q1 ? ++orientation : --orientation, tool_size / 2)) {
    ...
    if (!see_obstacle(orientation, tool_size / 2)) {
      ...
      if (!see_obstacle(quadrant == q4 ? ++orientation : --orientation, tool_size / 2)) {
...
bool FullMstcOnline::visit(CellPtr cell, Quadrant quadrant, bool flexibly) {
  PartiallyOccupiableIdentifiableCellPtr c = boost::dynamic_pointer_cast<PartiallyOccupiableIdentifiableCell>(cell);
  c->set_current_quadrant(quadrant);
  return go_to(c->get_current_position(), flexibly);
}

bool FullMstcOnline::state_of_subcells_of(CellPtr cell, Orientation orientation) {
  PartiallyOccupiableIdentifiableCellPtr c = boost::dynamic_pointer_cast<PartiallyOccupiableIdentifiableCell>(
      *communicator->cells.find(boost::static_pointer_cast<IdentifiableCell>(cell)));
  Quadrant q;
  if (orientation == AT_LEFT_SIDE)
    ...
    q = IV;
  return (c->get_quadrants()[q] == NEW && c->get_quadrants()[+q] == OBSTACLE && c->get_quadrants()[-q] == OBSTACLE) ||
                 (c->get_quadrants()[--q] == NEW && c->get_quadrants()[+q] == OBSTACLE &&
                  c->get_quadrants()[-q] == OBSTACLE)
             ? DIAGONALLY_OPPOSITE_OBSTACLES
             : NON_DIAGONALLY_OPPOSITE_OBSTACLES;
}
}
}
}
