...
Space::Space(PointPtr center, double size_x, double size_y)
  ...
  points.insert(points.end(), PointPtr(new Point(center->x - size_x / 2, center->y + size_y / 2)));
  points.insert(points.end(), PointPtr(new Point(center->x + size_x / 2, center->y + size_y / 2)));
  points.insert(points.end(), PointPtr(new Point(center->x + size_x / 2, center->y - size_y / 2)));
  points.insert(points.end(), PointPtr(new Point(center->x - size_x / 2, center->y - size_y / 2)));
...
bool Space::compare_positions_x(boost::shared_ptr<Space> space1, boost::shared_ptr<Space> space2) {
...
bool Space::is_parent(boost::shared_ptr<Space> space1, boost::shared_ptr<Space> space2) {
  if (fabs((space1->get_center()->x + space1->get_size_x() / 2) -
    ...
    if ((space1->get_center()->y - space1->get_size_y() / 2 > space2->get_center()->y + space2->get_size_y() / 2) ||
        (space1->get_center()->y + space1->get_size_y() / 2 < space2->get_center()->y - space2->get_size_y() / 2)) {
      return false;
    }
    return true;
  }
...
PointPtr Space::get_center() { return center; }

double Space::get_size_x() { return size_x; }

double Space::get_size_y() { return size_y; }

SpacePtr Space::get_parent() { return parent; }

void Space::set_parent(SpacePtr parent) { this->parent = parent; }

void Space::set_point_backtrack(boost::shared_ptr<Space> space1, boost::shared_ptr<Space> space2, double robot_size) {
  if ((space1->get_center()->y - space1->get_size_y() / 2 > space2->get_center()->y - space2->get_size_y() / 2)) {
