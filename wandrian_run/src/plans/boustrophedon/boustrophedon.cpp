...
void Boustrophedon::dfs(SpacePtr space) {
  ...
  for (inspectLC = space->children.begin(); inspectLC != space->children.end();
    ...
    if ((*inspectLC)->status_visited == false) {
      ...
      if ((((*inspectLC)->get_center()->y * 2 - (*inspectLC)->get_sizey()
          + robot_size) / 2)
...
std::list<SpacePtr> Boustrophedon::create_list_space(ObstaclePtr environment,
  ...
  for (inspectLV = list_vertices.begin(), j = 1, i = 1;
    ...
    std::cout << "V" << j << "(" << (*inspectLV)->get_positions()->x << ", "
        << (*inspectLV)->get_positions()->y << " )" << std::endl;
    ...
    for (inspectLVT = listvertices_temp.begin();
      ...
      std::cout << "V" << "(" << (*inspectLVT)->get_positions()->x << ", "
          << (*inspectLVT)->get_positions()->y << " )" << std::endl;
    }
    ...
    if ((*inspectLV)->left_compared_center()
      ...
      for (inspectLVT = listvertices_temp.begin();
        ...
        if ((*inspectLVT)->get_positions()->y
            < (*inspectLV)->get_positions()->y) {
          vertices_previous = *inspectLVT;
          continue;
        }

        // Create space
        sizex = (*inspectLV)->get_positions()->x
            - (*inspectLVT)->get_positions()->x;
        sizey = (*inspectLVT)->get_positions()->y
            - vertices_previous->get_positions()->y;
        ...
        listvertices_temp.push_back(
            VerticesPtr(
                new Vertices(
                    PointPtr(
                        new Point(center_temp->x + sizex / 2,
                            center_temp->y + sizey / 2)),
                    ObstaclePtr(new Obstacle(center_temp, sizex, sizey)))));

        if ((*inspectLV)->get_positions()->y
            != environment->get_center()->y - environment->get_sizey() / 2) {
          listvertices_temp.push_back(
              VerticesPtr(
                  new Vertices(
                      PointPtr(
      ...
      for (inspectLVT = listvertices_temp.begin();
          inspectLVT != listvertices_temp.end(); ++inspectLVT) {
        std::cout << "V current " << (*inspectLV)->get_positions()->y
            << std::endl;
        std::cout << "V temp " << (*inspectLVT)->get_positions()->y
            << std::endl;
        if ((*inspectLV)->get_positions()->y
            == (*inspectLVT)->get_positions()->y) {
      ...
      sizex = (*inspectLV)->get_positions()->x
          - vertices_previous->get_positions()->x;
      sizey = (*inspectLVT)->get_positions()->y
          - vertices_previous->get_positions()->y;
                ...
                new Vertices(
                    PointPtr(
                        new Point(center_temp->x + sizex / 2,
                            center_temp->y + sizey / 2)),
                    ObstaclePtr(new Obstacle(center_temp, sizex, sizey)))));
        std::cout << center_temp->x + sizex / 2 << center_temp->y + sizey / 2
            << std::endl;
      } else {
        listvertices_temp.push_back(
            VerticesPtr(
      ...
      if ((*inspectLV)->get_positions()->x
          == environment->get_center()->x + environment->get_sizex() / 2) {
        ++inspectLV;
        ++j;
      }
    }
...
std::list<VerticesPtr> Boustrophedon::create_list_vertices(
  ...
  std::cout << "Environment : 0(" << environment->get_center()->x << " ,"
      ...
      << environment->get_sizex() << " ," << environment->get_sizey() << ")"
      << std::endl;
  ...
  for (inspectLP = list_point.begin(); inspectLP != list_point.end();
    ...
    for (inspectLV = list_vertices.begin(), j = 0;
      ...
      std::cout << "V" << (*inspectLV)->get_positions()->x << " "
          << (*inspectLV)->get_positions()->y << std::endl;

      if (((*inspectLV)->get_positions()->x == (*inspectLP)->x)
          && ((*inspectLV)->get_positions()->y == (*inspectLP)->y)) {
        j = 1;
...
//	ObstaclePtr environment;
  ...
  for (inspectLS = list_space.begin(), i = 1; inspectLS != list_space.end();
    ...
    std::cout << "Size(" << (*inspectLS)->get_sizex() << " ,"
        << (*inspectLS)->get_sizey() << " )" << std::endl;
    ...
    for (inspectLS_child = (*inspectLS)->children.begin(), j = 1;
      ...
      std::cout << "Size(" << (*inspectLS_child)->get_sizex() << " ,"
          << (*inspectLS_child)->get_sizey() << " )" << std::endl;
    }
    std::cout << " " << std::endl;
