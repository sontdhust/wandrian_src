...
SpiralStc::SpiralStc(EnvironmentPtr environment, PointPtr starting_point,
	...
	starting_cell = boost::shared_ptr<Cell>(
			new Cell(
...
//	std::cout << "\n";
	...
	PointPtr new_position = boost::shared_ptr<Point>(
...
//		getchar();
	...
	Vector2dPtr direction = boost::shared_ptr<Vector2d>(
			new Vector2d(
	...
	while (neighbors_count < 3) {
		...
		CellPtr neighbor = boost::shared_ptr<Cell>(
				new Cell(
						boost::shared_ptr<Point>(
								new Point(
