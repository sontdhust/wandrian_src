...
State FullSpiralStc::state_of(CellPtr cell_to_check) {
  std::set<CellPtr, CellComp>::iterator oc = old_cells.find(cell_to_check);
  State state = (oc != old_cells.end()) ? OLD : NEW;
  if (state == OLD)
...
void FullSpiralStc::scan(CellPtr current) {
  ...
  do {
    ...
    if (state_of(neighbor) == OLD) { // Old cell
      // Go to next sub-cell
      bool succeed = go_from(current, DONT_PASS, neighbor);
      continue;
    } else {
...
bool FullSpiralStc::go_from(CellPtr current, bool pass, CellPtr next) {
  ...
  if (quadrant == q1 || quadrant == q2) {
    if (!see_obstacle(orientation, robot_size / 2)) {
      ...
      if (!see_obstacle(quadrant == q1 ? ++orientation : --orientation,
        ...
        if (!see_obstacle(quadrant == q1 ? --orientation : ++orientation,
          ...
          n->set_quadrants_old(q1 ? q3 : q4);
          return false;
        }
      } else {
        c->set_quadrants_old(q1 ? q2 : q1);
        return false;
      }
    }
    ...
    if (!see_obstacle(orientation, robot_size / 2)) {
      ...
      c->set_quadrants_old(quadrant == q4 ? q1 : q2);
      if (!see_obstacle(quadrant == q4 ? ++orientation : --orientation,
        ...
        if (!see_obstacle(quadrant == q4 ? --orientation : ++orientation,
          ...
          c->set_quadrants_old(quadrant == q4 ? q2 : q1);
          return false;
        }
      } else {
        c->set_quadrants_old(quadrant == q4 ? q3 : q4);
        return false;
      }
    }
  } else
