...
void FullSpiralStc::scan(CellPtr current) {
  ...
  do {
    ...
    if (check(neighbor) == OLD) { // Old cell
      ...
      bool succeed = go_across(current, orientation % VectorPtr());
      if (!succeed) { // Obstacle
  ...
  if (!is_starting_cell) {
    go_across(current, orientation % VectorPtr());
  }
...
bool FullSpiralStc::go_across(CellPtr cell, Orientation orientation) {
  Quadrant quadrant = cell->current_quadrant;
  VectorPtr o = ~orientation;
  switch (orientation) {
  case AT_RIGHT_SIDE:
    if (quadrant == IV) {
      if (see_obstacle(o, (robot_size / 2) / STEP_SIZE)) {
        return false;
      } else {
        go_to(cell->get_current_position() + o, STRICTLY);
      }
    }
    break;
  case IN_FRONT:
